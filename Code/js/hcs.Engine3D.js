...
hcs.Engine3D = function() {
    var t, e = function(e, n, i) {
        this._initialized = false, this._components = new wnp.ComponentCollection(wanaplan), this.hardwareScaling = 1, this.contextLostCount = 0, this.MAX_CONTEXT_LOST = 5, this.MODE_NORMAL = 1, this.MODE_DRAG = 2, this.MODE_PAINT = 4, this.MODE_CONTEXTMENU = 8, this.mode = this.MODE_NORMAL, this.container = e, this.canvas = document.createElement("canvas"), e.appendChild(this.canvas), this.canvas.width = wanaplan.getWidth(), this.canvas.height = wanaplan.getHeight();
        var o = wanaplan.configuration && wanaplan.configuration.useAntialiasing === false ? false : true;
        this.engine = new BABYLON.Engine(this.canvas, o), this.scene = new BABYLON.Scene(this.engine), this.scene.ambientColor = new BABYLON.Color3(1, 1, 1), this.scene.clearColor.copyFromFloats(.96, .96, .96), this.shadowGenerator = null, this.worldPlane = new BABYLON.Plane(0, 1, 0, 0), this.enabled = false, this.viewInitialized = false, this.isViewer = "undefined" != typeof i ? i : false, this.canvas.addEventListener("webglcontextlost", this.onContextLost.bind(this), false), this.canvas.addEventListener("webglcontextrestored", this.onContextRestored.bind(this), false), this.canDraw = true, t = this, n || (this.structure = wanaplan.structure, this.materialFactory = new wnp.MaterialFactory(wanaplan.configuration), this.cameraFeatures = new wnp.CameraFeatures, this.pointerManager = new wnp.PointerManager(wanaplan, this.onMouseEvent.bind(this), this.canvas), this.pointerManager.touchManager.setDeadZone(1), this.keyboardManager = wanaplan.keyboardManager, this.stats = new Stats, this.stats.domElement.style.position = "absolute", this.stats.domElement.style.bottom = "28px", this.stats.domElement.style.left = "0px", this.stats.domElement.style.zIndex = "99999", this.stats.domElement.style.display = "none", wanaplan.configuration.useStats && (this.stats.active = true), document.body.appendChild(this.stats.domElement), document.addEventListener("wnp.engine3D.stats.activeChanged", function(e) {
            t.stats.active = e.value, t.stats.domElement.style.display = e.value ? "block" : "none", wanaplan.configuration.useStats = e.value ? true : false, wanaplan.configuration.saveConfiguration()
        }, false), document.addEventListener("wnp.engine3D.refreshGL", function() {
            var t = location.hash.replace("#", ""), e = atob(t), n = btoa(e.replace("&planUrl=:new", "&planUrl"));
            location.hash = n, window.location.reload(!0)
        }, false), document.addEventListener("wnp.engine3D.showProducts", function(t) {
        ...
        }, false), this.isViewer || (document.addEventListener("wnp.engine3D.click", t.onClick, false), document.addEventListener("wnp.engine3D.double-click", t.onDoubleClick, false), document.addEventListener("pointerdown", t.onMouseDown, false)), document.addEventListener("wnp.engine3d.wallsReady", this.onWallsReady, false)), this.getContainer = function() {
            return this.container
        }
    };
    return e.prototype.collideWithMeshes = function(e, n, i) {
        var o = t.scene.createPickingRay(e, n), r = o.intersectMeshes(i, true, true);
        return r
        ...
        if (!this.viewInitialized) {
            ...
            t = e ? this.cameraFeatures.getBestFocusRadius(e, wanaplan.engine3D.camera, wanaplan.engine3D.scene) : 500, wanaplan.engine3D.camera.radius = t, wanaplan.engine3D.camera.alpha = -120 / 180 * Math.PI, wanaplan.engine3D.camera.beta = 60 / 180 * Math.PI, this.viewInitialized = true
        }
        ...
        return this._components.removeComponent(t, true)
    }, e.prototype.clearComponents = function() {
        this._components.clear()
    }, e.prototype.collideWithScene = function(e, n) {
        var i = t.scene.createPickingRay(e, n), o = i.intersectMeshes(t.scene.meshes, true, true);
        return o
        ...
        wanaplan.configuration.useMultiTexturing || (BABYLON.StandardMaterial.SpecularTextureEnabled = false, BABYLON.StandardMaterial.BumpTextureEnabled = false, BABYLON.StandardMaterial.ReflectionTextureEnabled = false), this._components.initialize(), this._initialized = true
