...
const vec4 vSpecularColor = vec4(0.2, 0.2, 0.2, 5);

uniform vec4 vDiffuseColor;

#ifdef METAL
const vec3 vAmbientColor = vec3(0.0, 0.0, 0.0);
#endif

#ifdef GLASS
const vec3 vAmbientColor = vec3(0.2, 0.2, 0.2);
#endif
...
void main(void) {
	...
	#ifdef METAL
	vec4 color = vec4( (0.5*reflectionColor + diffuseColor + vAmbientColor)*1.3 + finalSpecular + finalDiffuse, alpha);
	#endif

	#ifdef GLASS
	vec4 color = vec4( 2.0*diffuseColor + 1.5*diffuseColor * (reflectionColor + vAmbientColor), alpha);
	#endif
	gl_FragColor = color;
}