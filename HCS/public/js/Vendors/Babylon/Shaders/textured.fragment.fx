...
#ifdef WOOD
const vec4 vSpecularColor = vec4(0.2, 0.2, 0.2, 20);
const vec3 vAmbientColor = vec3(2.0, 2.0, 2.0);
#endif

#ifdef LEATHER
const vec4 vSpecularColor = vec4(1.0, 1.0, 1.0, 400);
const vec3 vAmbientColor = vec3(1.0, 1.0, 1.0);
#endif

#ifdef TILE
const vec4 vSpecularColor = vec4(0.8, 0.8, 0.8, 400);
const vec3 vAmbientColor = vec3(0.7, 0.7, 0.7);
#endif

#ifdef MATT
const vec4 vSpecularColor = vec4(0, 0, 0, 400);
const vec3 vAmbientColor = vec3(0.8, 0.8, 0.8);
#endif
...
void main(void) {
	...
	//vec3 finalDiffuse = clamp(diffuseBase + vAmbientColor, 0.0, 1.0) * baseColor.rgb;
	vec3 finalSpecular = specularBase * specularColor;

	//vec4 color = vec4((finalDiffuse + finalSpecular + baseColor.rgb*0.3)*vAmbientColor, 1.0);
	vec4 color = vec4((finalDiffuse + finalSpecular + baseColor.rgb*0.8), alpha);
